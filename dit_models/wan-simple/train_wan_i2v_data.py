...
class TextVideoDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, metadata_path, max_num_frames=81, frame_interval=1, num_frames=81, height=480, width=832, is_i2v=False):
        ...
        metadata = []
        skipped_count = 0  # 用于记录被跳过的数据数量

        with open(metadata_path, "r") as f:
            for line in f:
                data = json.loads(line)

                # 使用 index 字段构建唯一 tensor 文件名
                index = data.get("index")
                if not index:
                    continue  # 如果没有 index，跳过
                
                # 构建目标 tensor 路径
                tensor_path = os.path.join(
                    "PATH_TO_PROCESSED_VIDEOS_DIR",
                    f"{index}.mp4.tensors.pth"
                )
        ...
        print(f"共跳过 {skipped_count} 条已存在的 tensor 数据。")
        ...
        self.text = [data["prompt"] for data in metadata]
        self.indexid = [data["index"] for data in metadata]
    ...
    def load_frames_using_imageio(self, file_path, max_num_frames, start_frame_id, interval, num_frames, frame_process):
        ...
        if reader.count_frames() < max_num_frames or reader.count_frames() - 1 < start_frame_id + (num_frames - 1) * interval:
            ...
            for frame_id in range(min(num_frames, reader.count_frames())):
                frame = reader.get_data(min(start_frame_id + frame_id * interval, last_frame_id))
        ...
        # 在total帧中均匀抽取num_frames帧
        total = reader.count_frames()
        # 生成 num_frames 个从 0 到 total-1 的均匀整数索引
        indices = np.linspace(0, total - 1, num_frames, dtype=int)
        frames = []
        first_frame = None
        for frame_id in indices:
            frame = reader.get_data(start_frame_id + int(frame_id))
            frame = Image.fromarray(frame)
            frame = self.crop_and_resize(frame)
            if first_frame is None:
                first_frame = frame
    ...
    def __getitem__(self, data_id):
        ...
        if self.is_i2v:
            ...
            data = {"text": text, "video": video, "path": path, "first_frame": first_frame,"indexid":indexid}
        else:
            data = {"text": text, "video": video, "path": path,"indexid":indexid}
        return data
...
class LightningModelForDataProcess(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        text, video, path, indexid = batch["text"][0], batch["video"], batch["path"][0], batch["indexid"][0]
        ...
        if video is not None:
            ...
            print(f"save to {path}.tensors.pth")
